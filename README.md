...
```sh
npm install telescope-decoders
```
## Table of contents

- [telescope-decoders](#telescope-decoders)
  - [Install](#install)
...
```js
import { cosmos } from 'telescope-decoders';
...
Import the `cosmos` object from `telescope-decoders`. 
...
```js
import { cosmwasm } from "telescope-decoders";
...
```js
import { ibc } from 'telescope-decoders';
...
```js
import { getSigningcosmosClient } from 'telescope-decoders';
...
Contract schemas live in `./contracts`, and protos in `./proto`. Look inside of `scripts/codegen.js` and configure the settings for bundling your SDK and contracts into `telescope-decoders`:
